...
const completeGraphToMinimumNodes = (nodes, edges, minimumNodes = 60) => {
  ...
  console.log(
    `🔄 Modo Manual: Solo hay ${nodes.length} nodos. Completando a ${minimumNodes}...`
  );
    ...
    do {
      ...
      Array.from(existingPositions).some((pos) => {
        ...
        const distance = Math.sqrt(
          Math.pow(x - newX, 2) + Math.pow(y - newY, 2)
        );
        return distance < 50;
      })
    );
    ...
    if (existingNodeIds.length > 0) {
      ...
      for (let j = 0; j < connections; j++) {
        ...
        do {
          targetNode =
            existingNodeIds[
              Math.floor(Math.random() * existingNodeIds.length)
            ];
          attemptsConnection++;
  ...
  console.log(
    `✅ Se agregaron ${nodesToAdd} nodos automáticamente. Total: ${newNodes.length} nodos`
  );
  return { nodes: newNodes, edges: newEdges };
};

export default forwardRef(function GraphPlayToolbar(
  { graphCanvasRef, onAttemptsUpdate, onSelectedAttemptChange, onReset },
  ref
) {
  ...
  const handlePlayClick = () => {
    ...
    setTimeout(() => {
      ...
      const algorithmName =
        currentAlgorithm === "las_vegas" ? "Las Vegas" : "Monte Carlo";
      ...
      if (isManualMode && nodes.length < 60) {
        console.log(
          `📊 Modo Manual detectado. Nodos actuales: ${nodes.length}. Necesarios: 60`
        );
      ...
      // Si aún no hay grafo, generar uno aleatorio (140–150 nodos)
      ...
      // 🔁 SIEMPRE reiniciamos los intentos para cualquier algoritmo
      const newBatch = [];
      ...
      for (let i = 0; i < numberOfAttempts; i++) {
        ...
        const coloredNodes = baseNodes.map((n) => {
          ...
          const displayColor =
            COLOR_MAP[colorName] || colorName || "#e74c3c";
        ...
        // ID de intento SIEMPRE 1..N en cada ejecución
        const attemptId = i + 1;
      ...
      const avgExecutionTime =
        (batchEndTime - batchStartTime) / numberOfAttempts;
      // (si quieres usar avgExecutionTime para algo, ya está calculado)

      // 🔁 Reemplaza completamente los intentos para esta ejecución
      const updatedAttempts = newBatch;
  ...
  const handleSelectAttempt = (e) => {
    const index = parseInt(e.target.value, 10);
    if (!Number.isNaN(index)) {
      handleLoadAttempt(index);
    }
  };

  const canPlay = selectedColors.length === colorCount;

  // Para etiquetar el mejor intento en el combo
  const bestConflicts =
    attempts.length > 0
      ? Math.min(...attempts.map((a) => a.totalConflicts))
      : null;
    ...
    <div className="graph-play-toolbar">
      ...
      {selectedAlgorithm === "monte_carlo" && (
        <div className="option-select">
          ...
          <input
            ...
            onChange={(e) =>
              setAcceptanceProbability(parseFloat(e.target.value))
            }
      ...
      <div className="option-select">
        ...
        <select
          ...
          onChange={(e) =>
            setNumberOfAttempts(parseInt(e.target.value))
          }
          disabled={isExecuting}
        >
      ...
      {/* Intentos como COMBOBOX */}
      <div className="attempts-panel">
        <h4>Intentos guardados</h4>
        {attempts.length === 0 ? (
          <p className="attempts-empty">Todavía no hay intentos</p>
        ) : (
          <div className="attempts-select-wrapper">
            <label htmlFor="attempt-select">Seleccionar intento:</label>
            <select
              id="attempt-select"
              className="attempts-combobox"
              value={
                currentAttemptIndex !== null ? currentAttemptIndex : ""
              }
              onChange={handleSelectAttempt}
              disabled={isExecuting}
            >
              <option value="" disabled>
                -- Seleccione un intento --
              </option>
              {attempts.map((att, idx) => {
                const isBestAttempt =
                  bestConflicts !== null &&
                  att.totalConflicts === bestConflicts;
                const labelExtra = isBestAttempt
                  ? " (mejor)"
                  : att.hasConflicts
                  ? " (con conflictos)"
                  : " (sin conflictos)";
                return (
                  <option key={att.id} value={idx}>
                    {`Intento ${att.id} – ${att.timestamp}${labelExtra}`}
                  </option>
                );
              })}
            </select>
