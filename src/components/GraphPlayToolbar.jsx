...
// ===== FUNCIÓN NUEVA: Completar grafo a mínimo de 60 nodos en modo manual =====
const completeGraphToMinimumNodes = (nodes, edges, minimumNodes = 60) => {
  ...
  // Usar posiciones existentes para evitar superposición
  ...
  // Generar nodos nuevos
  for (let i = 0; i < nodesToAdd; i++) {
    let attempts = 0;
    let newX, newY;
    
    // Buscar posición que no se superponga mucho (mínimo 50px de distancia)
    do {
      newX = 50 + Math.random() * 700; // Rango amplio para el canvas
      ...
      Array.from(existingPositions).some(pos => {
        ...
        return distance < 50; // Mínimo 50px entre nodos
      })
    );
    ...
    newNodes.push({
      ...
      data: { 
        ...
        isAutoGenerated: true // Marcar como generado automáticamente
      }
    });
    
    // Guardar posición redondeada para evitar superposición futura
    const roundedX = Math.round(newX / 20) * 20;
    const roundedY = Math.round(newY / 20) * 20;
    existingPositions.add(`${roundedX},${roundedY}`);
    
    // Conectar el nuevo nodo con algunos nodos existentes (2-3 conexiones)
    if (existingNodeIds.length > 0) {
      ...
      for (let j = 0; j < connections; j++) {
        ...
        // Evitar conectar con el mismo nodo múltiples veces
        do {
  ...
  // Conectar algunos nodos nuevos entre sí para formar subgrafos
  const newNodesAdded = newNodes.slice(nodes.length).map(n => n.id);
  if (newNodesAdded.length >= 3) {
    // Crear algunos pequeños grupos conectados
...
export default forwardRef(function GraphPlayToolbar({ graphCanvasRef, onAttemptsUpdate, onSelectedAttemptChange, onReset }, ref) {
  ...
  const handlePlayClick = () => {
    ...
    setTimeout(() => {
      ...
      // ===== NUEVA LÓGICA: Completar a 60 nodos si estamos en modo manual =====
      ...
      if (isManualMode && nodes.length < 60) {
        ...
        // Actualizar el canvas con los nodos agregados
